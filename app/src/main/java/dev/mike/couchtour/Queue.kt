 ...
 * collide with a phish.in one, so existing rows keep working untouched. [LOCAL_PLAYLIST] gets
 * its own prefix rather than reusing [PLAYLIST]'s for the same reason (#12, D161): a local id
 * would otherwise route into [PhishInApi.playlist], which has no such slug.
 */
...
fun localPlaylistQueueKey(id: String) = LOCAL_PLAYLIST_PREFIX + id
...
fun parseQueueKey(raw: String): QueueRef? = when {
    raw.startsWith(LOCAL_PLAYLIST_PREFIX) ->
        raw.removePrefix(LOCAL_PLAYLIST_PREFIX).takeIf { it.isNotEmpty() }
            ?.let { QueueRef(QueueKind.LOCAL_PLAYLIST, it) }
