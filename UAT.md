...
## macOS YouTube video playback (#231)

- [ ] `uat-056` **YouTube video plays through embedded player** (macOS) — Requires a YouTube Data API key: `defaults write dev.mike.couchtour.mac youtubeAPIKey <key>`. On the Phish artist page (Artists → Phish), the YouTube section lists videos. Click a video — the Now Playing inspector (or player rail right pane) shows the YouTube IFrame embedded player instead of the audio waveform/artwork. The video auto-plays visibly. Verify: (a) video plays with audio and picture, (b) the position timestamp in the rail/expanded view updates as the video plays, (c) the "YOUTUBE" eyebrow label and video title appear below the embed, (d) collapsing the Now Playing inspector leaves the video playing in the player rail, (e) starting a phish.in audio show replaces the YouTube surface with the normal audio waveform/artwork, (f) the YouTube video appears as a row in Continue Listening after playback.
