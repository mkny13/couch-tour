...
### D268 — Android volume leveling: AudioProcessor gain in PlaybackService, decode-ahead measurement in VolumeLeveler (#267)
...
- **Tests:** 25 unit tests in `VolumeLevelingTest` (gain math, int16 clamping, float PCM, 50ms ramp, track spread, HTTP Range pooling and fallback) and `VolumeLevelerTest` (cache hit/miss, stale version, failed measurement leaving no row, dynamic update, disabled setting); Android suite at 636.
