...
### D252 — macOS YouTube playback is visible-only via the official IFrame Player API (#231)

Owner decision (issue #231 comment): **visible-only**. Background audio-only playback and any audio↔video toggle are dropped — YouTube's embedded-player Terms of Service prohibit hidden/background players, prohibit separating audio from video, and prohibit direct stream extraction. The implementation:
- **IFrame Player API (`YouTubePlayerView.swift`):** a `WKWebView` wrapper loading `https://www.youtube.com/iframe_api` — YouTube's official, documented, no-API-key-needed embed. Auto-plays on appear, reports play/pause/position/duration to `Player` through a `WKScriptMessageHandler` JavaScript bridge. No undocumented or unofficial APIs.
- **`youtube:<videoId>` queue key:** a new namespace in `QueueKey.swift` (alongside `show:`, `playlist:`, `relisten:`, `local-playlist:`). Parsed by macOS `parseQueueKey`; an Android client that does not yet recognize the prefix skips the row (returns `nil`) rather than mis-playing it, safe for the eventual shared `progress` table.
- **`PlaybackMode` (.audio / .youtubeVideo):** pure enum in `CouchTourKit`, testable via `swift test`. `Player.playbackMode` drives the UI to show either the WKWebView embed (YouTube) or the AVQueuePlayer waveform+transport (audio). Starting a show resets to `.audio`; starting a YouTube video sets `.youtubeVideo`.
- **Progress persistence:** YouTube position is saved through the same `ProgressRecorder.saveTick` path as audio, under the `youtube:` key. The progress row appears in Continue Listening, though re-entering the video requires the player rail or Now Playing inspector to be visible (the IFrame must be on screen — visible-only).
- **Supersedes nothing:** this is the first YouTube playback implementation. ROADMAP #16's "audio-only background playback" wording is updated to match the visible-only outcome.
