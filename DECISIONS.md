...
- **F2: the returned cursor was computed from a `seqs.next` read taken before `applyIncomingChanges`'s atomic bump.** If another device in the group pushed in the gap between that read and the response being assembled, the rows returned could carry seqs higher than the cursor reported, and the client would re-pull the same rows on every subsequent sync. The response `seq` is now the seq of the last row in the (already seq-ordered) result set, falling back to `since` when there are none — derived from the same fresh read that produces the rows, not a value cached earlier in the request. Regression test forces the interleaving deterministically (delaying the final `SELECT` via a patched `env.DB.prepare` until a second device's push commits, rather than hoping two real concurrent requests race the right way) and confirms a follow-up sync at the returned cursor returns nothing.
...
- **Tests:** 12 new vitest tests in `sync/test/sync.test.ts` covering the four fixes above plus the existing happy paths (pair → claim → push/pull round trip, 410 on a stale cursor with the `since = 0` exemption). Android suite unchanged at 641, macOS unchanged at 443.
