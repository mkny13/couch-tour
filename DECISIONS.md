...
- **Tests:** `YouTubeCatalogTest` (MockWebServer, channel-resolution/hide rules, source wiring) and `ArtistScreenTest` — the app's first Robolectric Compose UI tests, which pull in `ui-test-junit4`/`ui-test-manifest` (BOM-versioned). Section states verified: rendered rows, tap→video id, hidden (no channel / no key), inline error on HTTP failure, "No videos." on an empty channel.
